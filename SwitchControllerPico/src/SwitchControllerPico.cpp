// Pico SDK用 delay_with_tud_task関数
#include "pico/stdlib.h"
#include "tusb.h"
void delay_with_tud_task(uint32_t ms)
{
  uint32_t start = to_ms_since_boot(get_absolute_time());
  uint32_t last_tud = start;
...
*/
void switchcontrollerpico_init(void)
{
...
*/
void switchcontrollerpico_init(uint16_t button_pushing_msec)
{
  _button_pushing_msec = button_pushing_msec;
  tusb_init();
  while (!tud_ready())
...
*/
void switchcontrollerpico_reset(void)
{
...
void pushButton(Button button, int delay_after_pushing_msec, int loop_num = 1)
{
  for (int i = 0; i < loop_num; i++)
  {
    ...
    delay_with_tud_task(_button_pushing_msec);
    SwitchController().releaseButton(button);
    delay_with_tud_task(delay_after_pushing_msec);
  }
}
...
void pushButton2(Button button, int pushing_time_msec, int delay_after_pushing_msec, int loop_num = 1)
{
  ...
  {
    SwitchController().pressButton(button);
    delay_with_tud_task(pushing_time_msec);
    SwitchController().releaseButton(button);
    delay_with_tud_task(delay_after_pushing_msec);
  }
}
...
{
  double rad = (double)direction_deg * 3.14159265358979323846 / 180.0; // 弧度法(ラジアン)変換
  // int x, y;
  int lx_per;
  int ly_per;
  ...
  if (holdtime > 0)
  { // holdtime=0のときは押しっぱなし。
    delay_with_tud_task(holdtime);
    // SwitchControlLibrary().MoveLeftStick(128, 128); // 傾きを直す
    SwitchController().setStickTiltRatio(0, 0, 0, 0);
  }
  if (delaytime > 0)
    if (delaytime > 0)
      delay_with_tud_task(delaytime);
  return;
}
...
void tiltJoystick(int lx_per, int ly_per, int rx_per, int ry_per, int tilt_time_msec, int delay_after_tilt_msec)
{
  ...
  delay_with_tud_task(tilt_time_msec);
  if (delay_after_tilt_msec > 0)
  {
    SwitchController().setStickTiltRatio(0, 0, 0, 0);
    delay_with_tud_task(delay_after_tilt_msec);
  }
}
